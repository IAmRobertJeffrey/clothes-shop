...
{
	return (
		<Box display="flex" boxSizing="border-box" flexDirection="column" bg="gray.700" rounded="lg" overflow="hidden" shadow="lg" w="100%" h="500px">
			<img style={{ objectFit: "cover", height: "50%", width: "100%" }} alt="clothing" key={current.id} src={current.image} />

			<Box display="flex" p="5" isTruncated justifyContent="space-between">{current.product_name} <Button colorScheme="teal">{current.sex === "male" && "Men"}{current.sex === "female" && "Women"}{current.sex === "unisex" && "Unisex"}</Button></Box>
